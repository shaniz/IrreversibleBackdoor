...
def save_data(save_path,
    ...
    with open(save_path + '/' + 'finetune_restrict_test.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['total loop', 'finetune restrict test acc', 'finetune restrict test loss'])
        for i, j, k in zip(total_loop_index, all_finetune_restrict_test_acc, all_finetune_restrict_test_loss):
            writer.writerow([i, j, k])

    with open(save_path + '/' + 'final_test.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['final original test acc', 'final finetune restrict test acc', 'final finetune restrict test loss'])
        writer.writerow([final_orig_test_acc, final_finetune_restrict_test_acc, final_finetune_restrict_test_loss])
