...
def get_dataset(dataset, data_path, subset="imagenette", args=None):
    if dataset == 'MNIST':
        ...
        testset = datasets.ImageFolder(root=data_path + '/val/',transform=val_transforms)
        ...
        trainset = datasets.ImageFolder(root=data_path + '/train/',transform=train_transforms)
...
def test(model, original_testloader, device):
    """
    Calculate model accuracy+loss on testset
    """
    test_loss = 0
    correct = 0
    total = 0
