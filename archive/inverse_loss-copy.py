...
from utils import save_bn, load_bn, accuracy, get_pretrained_model, test_original, test, set_seed, save_data, initialize
...
def args_parser():
    ...
    parser.add_argument('--test_iterval', default=50, type=int)
    parser.add_argument('--arch', default='caformer', type=str)
    parser.add_argument('--gpus', default='0,1', type=str)
    parser.add_argument('--datasets', default='', type=str, choices=['CIFAR10', 'MNIST', 'SVHN', 'STL', 'CINIC'])
...
def test_finetune(model, trainset, testset, epochs, lr):
    ...
    trainloader = DataLoader(trainset, batch_size=256, shuffle=True, num_workers=4, drop_last=True, persistent_workers=True)
    testloader = DataLoader(testset, batch_size=256, shuffle=False, num_workers=4, drop_last=True, persistent_workers=True)
...
def main(
    ...
    # original domain
    orig_trainset, orig_testset = get_dataset('ImageNet', '../imagenette2/', subset='imagenette', args=args)
    ...
    # restricted domain
    target_trainset, target_testset = get_dataset(args.dataset, '../../../datasets', args=args)
    target_trainloader = DataLoader(target_trainset, batch_size=args.bs, shuffle=True, num_workers=4, drop_last=True, persistent_workers=True)
    target_testloader = DataLoader(target_testset, batch_size=args.bs, shuffle=False, num_workers=4, drop_last=True, persistent_workers=True)
    
    original_iter = iter(orig_trainloader)
    ...
    finetuned_target_test_acc = []
    finetuned_target_test_loss = []
    
    final_orig_test_acc = []
    final_finetuned_test_acc = []
    final_finetuned_test_loss = []
    ...
    model = nn.DataParallel(model)
    test_original(model, orig_testloader, device)
    ...
    for i in range(1,total_loop+1):
        ...
        for nl in  range(args.nl_loop):
            ...
            orig_train_loss.append(round(loss.item(),2))
            natural_optimizer.step()
            acc, loss = test_original(model, orig_testloader, device)
            orig_test_loss.append(loss)
            orig_test_acc.append(acc)
        ...
        if (i+1) % args.test_iterval == 0:
            ...
            finetuneacc, finetunetest_loss = test_finetune(test_model, target_trainset, target_testset, args.finetune_epochs, args.finetune_lr)
            print(f'finetune outcome: test accuracy is{finetuneacc}, test loss is{finetunetest_loss}')  

            finetuned_target_test_acc.append(finetuneacc)
            finetuned_target_test_loss.append(finetunetest_loss)
...
## test the original accuracy
    print('===============Test original==============')
    model = load_bn(model, means, vars)
    test_acc,_ = test_original(model, orig_testloader, device)
    final_orig_test_acc.append(test_acc)
## test finetune outcome
    print(f'**************Finally test truly finetune ({args.truly_finetune_epochs} epochs)***************')
    test_model2 = copy.deepcopy(model.module)
    finetune_test_acc, finetune_test_loss = test_finetune_final('our finetune/not init fc',test_model2, target_trainset, target_testset, args.truly_finetune_epochs, args.finetune_lr)
    print(f'Finally finetune outcome: test accuracy is{finetune_test_acc}, test loss is{finetune_test_loss}')
    final_finetuned_test_acc.append(finetune_test_acc)
    final_finetuned_test_loss.append(finetune_test_loss)
## save model
    ...
    save_data(save_path, queryset_loss, queryset_acc, orig_test_loss, orig_train_loss, orig_test_acc, finetuned_target_test_acc, finetuned_target_test_loss, final_orig_test_acc, final_finetuned_test_acc, final_finetuned_test_loss, total_loop_index, ml_index, nl_index)
